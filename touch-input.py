...
def process_img(frame, threshold, window_w, window_h):
    ...
    for cnt in contours:
        ...
        if area < 80 or area > 3500:
            ...
            # hover / light touch has smaller area -> red bounding box
            if 80 < area < 600:
                gesture = 'hover'
                cv2.rectangle(img_contours,(x,y),(x+w,y+h),(0, 0, 255),2)

            # hard touch has greater area -> green bounding box
            elif 600 < area < 3500:
                gesture = 'touch'
                cv2.rectangle(img_contours,(x,y),(x+w,y+h),(0, 255, 0),2)

            
            # this was also for ERWIN, but ERWIN still sucks
            ...
            try:
                ...
                # value = np.max(y)
                # print(gesture, value)
                streamer.add_to_stream(type=gesture, x=cX, y=cY)
            except:
                continue
...
while True:
    ...
    if not running:
        ...
        img_gray = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)
        threshold = np.bincount(img_gray.flatten()).argmax() - 60
    else:
